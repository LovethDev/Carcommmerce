...
export const Header: React.FC = () => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-gray-900 to-gray-800 shadow-lg border-b border-gray-700 font-inter">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          {/* Logo and Brand Name */}
          <div className="flex items-center">
            <Car className="h-8 w-8 text-red-500 mr-2 animate-pulse" /> {/* Added a subtle pulse animation */}
            <Link
              to="/"
              className="text-xl font-extrabold text-white hover:text-red-300 transition-colors duration-300 ease-in-out flex items-center gap-2"
            >
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="text-gray-300 hover:text-white focus:outline-none focus:ring-2 focus:ring-inset focus:ring-red-500 p-2 rounded-md"
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-gray-900 bg-opacity-95 backdrop-blur-sm flex flex-col items-start p-6 animate-slideInLeft">
          <div className="flex justify-between items-center w-full mb-8">
            <Link to="/" className="text-xl font-bold text-white flex items-center gap-2" onClick={handleCloseMobileMenu}>
              <Car className="h-7 w-7 text-red-500" /> BDA
            </Link>
            <button
              onClick={handleCloseMobileMenu}
              className="text-gray-400 hover:text-white p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-inset focus:ring-red-500"
            >
              <X className="h-6 w-6" aria-label="Close menu" />
            </button>
          </div>
